...
export default function PreserveLayoutPdfButton({
  ...
  const handleExport = async () => {
    try {
      ...
      // Directly call the internal PDF service (only reachable from campus/VPN)
      const resp = await fetch('http://10.3.58.3:3001/generate-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // Include credentials so the browser sends cookies for authentication
        credentials: 'include',
