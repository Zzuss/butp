...
interface CoursesRadarChartProps {
  data: number[]
  className?: string
}

export function CoursesRadarChart({ 
  data, 
  className = ""
}: CoursesRadarChartProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      const angle = (i * 2 * Math.PI) / data.length - Math.PI / 2
      // 数值映射：假设数据范围是0-100，映射到半径，使用缩放因子放大显示
    ...
    ctx.font = 'bold 10px Arial' // 缩小字体以适应18个标签
