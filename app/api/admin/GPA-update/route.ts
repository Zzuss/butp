...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('开始更新GPA数据...');

    // 调用专门的GPA更新函数
    // 返回值类型：Array<{updated_count: number, inserted_count: number}>
    const { data, error } = await supabase.rpc('update_gpa_student');

    if (error) {
      console.error('执行GPA更新失败:', error);
      return NextResponse.json(
        { 
          error: '执行GPA更新失败', 
    ...
    console.log(`GPA更新完成: 更新 ${updatedCount} 条，插入 ${insertedCount} 条，总计 ${totalProcessed} 条`);
