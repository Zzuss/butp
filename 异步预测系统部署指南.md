...
server {
    ...
    # 预测API路由到8082（处理大文件和长时间请求）
    location /api/predict {
        proxy_pass http://127.0.0.1:8082/api/predict;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        ...
        proxy_send_timeout 1500s;        # 发送超时 25分钟
        proxy_read_timeout 1500s;        # 读取超时 25分钟
        ...
        # 防止upstream超时
        proxy_next_upstream_timeout 1500s;
...
```bash
cd /root/function_aliyun
nohup python3 app.py --port 8082 > prediction_8082.log 2>&1 &
```

### 启动8083端口服务（轻量服务）
```bash
cd /root/function_aliyun
nohup python3 lightweight_server.py --port 8083 > lightweight_8083.log 2>&1 &
```
...
# 查看端口占用情况
netstat -tlnp | grep -E ':(8082|8083|8080)'
...
```bash
# 查看预测API日志
tail -f /root/function_aliyun/prediction_8082.log

# 查看轻量服务日志
tail -f /root/function_aliyun/lightweight_8083.log
